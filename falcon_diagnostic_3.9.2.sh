#!/bin/bash
# Linux Diagnostic Script version 3.9.2 March 20, 2022
...
function usage
{
    echo "Usage: "$progname "[-h|--help] [-u|-unobtrusive]"
}
...
if [ -z $unobtrusive ]; then
    ...
    if [ -f "/opt/CrowdStrike/KernelModuleArchive" ]; then
            ...
            supported_kernels=`xz -dc /opt/CrowdStrike/KernelModuleArchive | strings | grep '^[2-4]\..*'`
    ...
    if [ $supported == 1 ]; then
        ...
        if [ -f "/opt/CrowdStrike/KernelModuleArchiveExt" ]; then
...
echo >> $readme_file
echo "With release of version 3.7 of falcon_diagnostics.sh:" >> $readme_file
...
if [ -z $unobtrusive ]; then
    ...
    if [ $same -eq 1 ]; then ln /var/log/falconctl.log; else cp /var/log//var/log/falconctl.log; fi                                                    &> $error_file
fi
...
if [ -z $unobtrusive ]; then
    ...
    command_array+=('echo t > /proc/sysrq-trigger'                                                                /dev/null)
    command_array+=('echo w > /proc/sysrq-trigger'                                                                /dev/null)
